...
from Retrain_modeltestwin1_batchnorm import Retrain_Tensorflow_Model
...
#        self.BASE2_PATH = "/Users/deyi/Google Drive/graduate seminar/Code/data"
        ...
        self.dims_output = 5
        self.leaning_rates = [0.0001, 0.00001, 0.00001]
...
#        with tf.device('/device:GPU:0'):
        #with tf.device('/cpu:0'):
            ...
            tf_model = Retrain_Tensorflow_Model(self.dims_image, self.dims_output) # CALCULATE dims_output
            _y =  tf_model.model(x)
...
##save model and parameters            
            ...
            saver.restore(self.sess, "/Users/sixge/Desktop/data/savedmodel.ckpt")            
...
#                    print("type is ",type(training_batch))
                batch_x, batch_y, label, trainnames =  self.get_x_y(training_batch)      
#                print(trainnames[1])
                ...
                #save all wrongly recongnized image infos to wrongimages
                predright = self.sess.run(pred_if_right, feed_dict={x: batch_x, y: batch_y})
                for l in range (0, self.GENERATOR_BATCH_SIZE):
                    if predright[l] == 0:
#                        print(trainnames[l])
                        wrongimages.append([trainnames[l], label[l]])
#                print(wrongimages)
...
#                        print('test_label', test_label)
                        batch_x, batch_y, testlabel, testnames=  self.get_x_y(testing_batch)
#                    self.sess.run(optimizer, feed_dict={x: batch_x, y: batch_y})
...
#            
            ...
            #Attach   wrong image names to new_trainLabels, so that it is duplicatedly trained in the future
            df = pd.read_csv("/Users/sixge/Desktop/data/new_trainLabels.csv")
            row_no = df.count()[1]
            for p in range(len(wrongimages)):
                df.loc[row_no + p] = wrongimages[p]  
