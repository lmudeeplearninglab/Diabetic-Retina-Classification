...
class dl_model():
    ...
    def __init__(self):
        ...
        self.MODEL_PATH = "/Users/sixge/Desktop/data/Models"
...
#        max = np.max(Y)
    ...
    def booleaner(self, data):
        cmpresult = cmpzero = [0]*len(data)
        for t in range(len(data)):
            cmpresult[t] = (data[t] != cmpzero[t])
            cmpresult = np.array(cmpresult)
            return cmpresult
...
#        print(data[:, 1])
#        print("type is", type(data[:, 1]))
        y = self.one_hot(self.booleaner(data[:, 1]))
#        print(y)
        z = data[:,2]
...
#            saver.save(self.sess, "/tmp/model.ckpt")
            saver.save(self.sess, "/Users/sixge/Desktop/data/Models/model_0&1234.ckpt")
